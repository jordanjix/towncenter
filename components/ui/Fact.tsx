...
export function Fact({
  ...
  const sourceNames = provenance.map((key) => sourceName(t, key)).join(" · ");
  ...
  return (
    <div className={cx("fact", empty && "fact--empty", className)}>
      <div className="fact__head">
        ...
        <span className="fact__gauge">
          <Gauge
            ...
            label={
              empty
                ? t("ui.fact.srNotRecorded", { name })
                : t("ui.fact.srScore", { name, value: bounded })
            }
          />
        </span>
      ...
      {empty ? (
        <p className="t-body-s fact__verbatim">{t("ui.fact.notRecorded")}</p>
      ...
      {provenance.length > 0 || surveyedOn ? (
        <p className="t-body-s fact__source">
          ...
          {stale ? <span className="fact__stale"> · {t("ui.fact.stale")}</span> : null}
        </p>
      ) : null}
    </div>
  );
