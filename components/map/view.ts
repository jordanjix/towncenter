...
export const SORT_OPTIONS: ReadonlyArray<{
  ...
  { key: "expectancy", labelKey: "list.sort.expectancy" },
  { key: "loot", labelKey: "list.sort.loot" },
  { key: "odds", labelKey: "list.sort.odds" },
  { key: "resistance", labelKey: "list.sort.resistance" },
  { key: "name", labelKey: "list.sort.name" },
  { key: "surveyed", labelKey: "list.sort.surveyed" },
];
...
export function sortLabel(t: Translate, sort: TargetSortKey): string {
  return t(
    SORT_OPTIONS.find((option) => option.key === sort)?.labelKey ??
      "list.sort.expectancy",
  );
}
